...
export default function VendorList () {
    ...
    const columns = [ {
        ...
        width: 150,
        Cell: row => (
            <Link to={ `${ process.env.PUBLIC_URL }/users/${ row.original.id }` } className="ml-1">
                <strong>{ row.value }</strong>
            </Link>
        )
    }, {
        Header: 'Store',
        accessor: 'vendor.store_name'
    }, {
        Header: 'Paypal Email',
        accessor: 'vendor.paypal_email',
        minWidth: 130
    }, {
        Header: 'Phone',
        ...
        Header: 'Featured',
        accessor: 'vendor.featured',
        Cell: row => (
            <div className="switch switch-sm switch-primary">
                <PtSwitch on={ row.value } onChange={ statusChanged } />
            </div>
        )
    }, {
        Header: 'Status',
    ...
    function fetchData ( state ) {
        setLoading( true );
        getUsers( state.page * state.pageSize, ( state.page + 1 ) * state.pageSize, [ ...state.filtered, { id: 'role_id', value: 4 } ], state.sorted )
            .then( results => {
                setLoading( false );
                setAjax( {
                    data: results.data,
    ...
    function statusChanged () {
        toast(
            <PNotify title="Successs" icon="fas fa-check" text="Vendor Status Changed." />,
            {
                containerId: "default",
    ...
    return (
        <>
            <Breadcrumb current="Vendors" paths={ [
                {
                    name: 'Home',
                    url: '/'
                }
