...
export default function ReviewList () {
    ...
    const columns = [ {
        ...
        id: 'product',
        Header: 'Product Name',
        accessor: d => { return { name: d.product.name, id: d.product.id } },
        minWidth: 130,
        sortable: true,
        Cell: row => (
            <Link to={ `${ process.env.PUBLIC_URL }/products/${ row.value.id }` }><strong>{ row.value.name }</strong></Link>
        )
    }, {
        Header: 'Rating',
        accessor: 'rating',
        ...
        Expander: ( { row } ) => (
            <>
                ...
                <Link to={ `${ process.env.PUBLIC_URL }/products/reviews/${ row._original.id }` } className="on-default edit-row mr-2"><i className="fas fa-pencil-alt"></i></Link>
    ...
    function fetchData ( state ) {
        ...
        author !== '' && filtered.push( { id: 'author', author } );
        setLoading( true );
        getReviews( state.page * state.pageSize, ( state.page + 1 ) * state.pageSize, filtered, state.sorted )
            .then( results => {
                setLoading( false );
                setAjax( {
                    data: results.data,
    ...
    function replyComment ( e, index ) {
        ...
        let newReply = {
            ...
            product: ajax.data[ index ].product,
            rating: 0,
            created_at: new Date()
        };
        let temp = [ ...ajax.data ];
        ...
        setExpanded( {} );
        toast(
            <PNotify title="Success" icon="fas fa-check" text="Reply added successfully." />,
            {
                containerId: "default",
    ...
    return (
        <>
            <Breadcrumb current={ 'Product Reviews' } paths={ [ { name: 'Home', url: '/' }, { name: 'Products', url: '/products' } ] } />

            <Row>
                <Col>
                    <Form method="get" onSubmit={ searchReviews }>
